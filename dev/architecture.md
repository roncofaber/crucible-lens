...
| `GET /samples?project_mfid={mfid}&project_scope={assigned|shared}&limit=1000&cursor=...` | Canonical project sample collections. Assigned results drive synchronization; Shared results are loaded on demand in Project Detail |
| `GET /datasets?project_mfid={mfid}&project_scope={assigned|shared}&limit=1000&cursor=...` | Canonical project dataset collections with the same cache boundary as project samples |
...
`ProjectDetailViewModel` keeps assigned and shared project collections in separate state. Assigned content follows the repository and synchronization path with offline fallback. Shared content is an explicit, on-demand API view with independent loading, refresh, and error state; it is not an offline replica and does not replace assigned content when it fails.
