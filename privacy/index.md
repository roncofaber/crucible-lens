...
The app lets you search for other Crucible users by name, and shows the name, username, and ORCID iD of project members, owners, and join requesters as returned by the Crucible API. This information is used only to display who you're looking at (e.g., to add them to a project you belong to) and is not shared with any third party beyond the Crucible API server itself.
...
If you're a member of a project, the app lets you add another Crucible user to it directly — either from that project's member list or from the user's own profile. This sends the other user's username to the Crucible API server to add them as a member of that project.
